...
void dot_part(const std::vector<double>& u, const std::vector<double>& v, int deb, int fin, double& scal){
...
{
  int num_thread = 2;
  std::vector<std::thread> threads; threads.reserve(num_thread);
  std::vector<double> partial_sums(num_thread);
  std::size_t dim_loc = u.size()/num_thread;
  int reste = u.size()%num_thread;
