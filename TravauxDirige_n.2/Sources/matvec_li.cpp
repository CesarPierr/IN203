...
int main( int nargs, char* argv[] )
{
    ...
    const int N = 8000;
    Matrix A(N);
    std::vector<double> u_rank( N );

    int nb_li = N/nbp;
    for ( int i = 0; i < N; ++i ) u_rank[i] = i+1;
    if (rank == 0){
        ...
        //std::cout << S << std::endl;
        MPI_Gather(v_rank.data(),nb_li,MPI_DOUBLE,buffer.data(),nb_li,MPI_DOUBLE,0,globComm);
