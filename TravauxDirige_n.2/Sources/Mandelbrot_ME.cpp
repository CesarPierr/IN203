...
int main(int argc, char *argv[] ) 
 { 
    ...
    if(rank == 0){
        ...
        std::vector<int>  MandelBrot(W*H);
        for(int i = 1; i<nbp;i++){
            MPI_Send(&count_task, 1, MPI_INT, i,0,globComm);
            task[i-1] = count_task;
            count_task++;
        }
        std::vector<int> buffer(W);
        while(count_task < 600){
            
            MPI_Recv(buffer.data(), W,MPI_INT, MPI_ANY_SOURCE,0,globComm,&Stat);
            int source = (int)Stat.MPI_SOURCE;
            MPI_Send(&count_task,1,MPI_INT,source,0,globComm);
            int line = task[source-1];
            for(int n=0; n<W; n++){
                MandelBrot[n+W*(H-line-1)] = buffer[n];
            }
            task[source-1] = count_task;
            count_task++;
        }
        count_task = -1;
        for(int i = 1; i<nbp; i++){
        ...
        savePicture("mandelbrot_ME.png", W, H,MandelBrot, maxIter);
        ...
        int numTask = 0;
        while(numTask != -1){
            MPI_Recv(&numTask,1,MPI_INT,0,0,globComm,&Stat);
            if (numTask >= 0){
                std::vector<int> pixels(W);
                computeMandelbrotSetRow(W, H, maxIter, numTask, pixels.data());
                MPI_Send(pixels.data(),W,MPI_INT,0,0,globComm);
            }
        }
    }
    MPI_Finalize();
