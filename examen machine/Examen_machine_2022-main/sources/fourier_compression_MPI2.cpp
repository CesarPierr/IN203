...
std::complex<double>* discretTransformFourier( std::uint32_t width, std::uint32_t height, unsigned char const* pixels, std::uint32_t begin, std::uint32_t size )
{
    ...
    for( std::uint32_t k1 = 0; k1 < ni; ++k1 )
    {
        ...
        {
            for (std::uint32_t n2 = begin; n2 < begin + size; ++n2 )
            {
...
int main(int nargs, char* argv[])
{
    ...
    unsigned char* compressed_img = inversePartialDiscretTransformFourier(sparse);
     ...
     elapsed_seconds_aff = end-start;
    std::cout << "temps discret inversePartialDiscret : " << elapsed_seconds_aff.count() <<std::endl;
