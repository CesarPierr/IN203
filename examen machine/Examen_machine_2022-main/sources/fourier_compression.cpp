...
sparseSpectralComposition compressSpectralComposition( std::uint32_t width, std::uint32_t height, const std::complex<double>* plainCoefs, double tauxCompression,double prct )
{
    std::uint32_t nb_coefs = std::uint32_t(prct*tauxCompression*width*height);
    struct node 
    {
...
int main(int nargs, char* argv[])
{
    ...
    std::uint32_t debut = rank * height/nbp;
    prct = height/nbp;
    if(rank == 0)
    {
        std::complex<double>* fourier = discretTransformFourier( width, height, image, ,h2);
        auto sparse = compressSpectralComposition(width, height, fourier, taux,prct);
