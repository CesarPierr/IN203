...
std::complex<double>* discretTransformFourier( std::uint32_t width, std::uint32_t height, unsigned char const* pixels ,std::uint32_t begin,  std::uint32_t h2)
{
    ...
    for( std::uint32_t k1 = begin; k1 < begin+h2; ++k1 )
    {
...
{
    std::chrono::time_point<std::chrono::system_clock> start, end, start_total, end_total;
    ...
    std::chrono::duration<double> elapsed_seconds_aff ;
    ...
    std::uint32_t h2 = rank== (nbp-1) ? height - (nbp -1)*height/nbp : height/nbp;

    std::complex<double>* rcv_buffer = new std::complex<double>[height*width];
    ...
    if (rank == 0)
    {
        ...
        std::complex<double>* fourier = discretTransformFourier( width, height, image,begin,h2);
        MPI_Reduce (fourier, rcv_buffer,height*width,MPI_C_DOUBLE_COMPLEX,MPI_SUM,0,globComm);
        end = std::chrono::system_clock::now();
        elapsed_seconds_aff = end-start;
        std::cout << "temps discret transform fourrier : " << elapsed_seconds_aff.count() <<std::endl;
        delete [] fourier;

    }
    else
    {
        std::uint32_t begin = rank*height/nbp;
        std::complex<double>* fourier = discretTransformFourier( width, height, image,begin,h2);
        MPI_Reduce (fourier, rcv_buffer,height*width,MPI_C_DOUBLE_COMPLEX,MPI_SUM,0,globComm);
        delete [] fourier;
    ...
    if(rank == 0)
    {
        ...
        start = std::chrono::system_clock::now();
        auto sparse = compressSpectralComposition(width, height, rcv_buffer, taux);
        end = std::chrono::system_clock::now();
        ...
        auto error = lodepng_encode24_file("compress_MPI1.png", compressed_img, width, height);
        ...
        elapsed_seconds_aff = end_total-start_total;
        std::cout << "temps total : " << elapsed_seconds_aff.count() <<std::endl;
    }
    delete [] rcv_buffer;
    delete [] image;
